...
-- Add support for Meegoda, Padukka, and Padukka new shops
-- ============================================================

-- 1. Add shop column to admins table
ALTER TABLE admins ADD COLUMN IF NOT EXISTS shop TEXT DEFAULT 'Meegoda';

-- 2. Add shop-specific quantities to inv_stock
ALTER TABLE inv_stock ADD COLUMN IF NOT EXISTS qty_meegoda INTEGER DEFAULT 0;
ALTER TABLE inv_stock ADD COLUMN IF NOT EXISTS qty_padukka INTEGER DEFAULT 0;
ALTER TABLE inv_stock ADD COLUMN IF NOT EXISTS qty_padukka_new INTEGER DEFAULT 0;

-- 3. Initialize the new columns with existing quantity if they are 0
-- Assuming existing stock is mostly in Meegoda for now
UPDATE inv_stock 
SET qty_meegoda = quantity 
WHERE qty_meegoda = 0 AND qty_padukka = 0 AND qty_padukka_new = 0;

-- 4. Create trigger to automatically sum total quantity
CREATE OR REPLACE FUNCTION update_total_quantity()
RETURNS TRIGGER AS $$
BEGIN
  NEW.quantity = COALESCE(NEW.qty_meegoda, 0) + COALESCE(NEW.qty_padukka, 0) + COALESCE(NEW.qty_padukka_new, 0);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_update_total_quantity ON inv_stock;
CREATE TRIGGER trg_update_total_quantity
  BEFORE INSERT OR UPDATE ON inv_stock
  FOR EACH ROW
  EXECUTE FUNCTION update_total_quantity();

-- 5. Update process_sale to handle shop-specific deduction
-- We drop the existing one and recreate it with the new parameter
DROP FUNCTION IF EXISTS process_sale(UUID, TEXT, TEXT, TEXT, NUMERIC, NUMERIC, TEXT, TEXT, JSONB);
