  ...
  try {
    ...
    try {
      if (data.user?.id && data.user?.email) {
        ...
        setImmediate(async () => {
          try {
            console.log('[signup] Starting background verification email for:', data.user.email);
            await sendEmail({
              to: data.user.email!,
            ...
            console.log('[signup] ✅ Background verification email sent successfully to:', data.user.email);
          } catch (bgError: any) {
            console.error('[signup] ⚠️ Background verification email FAILED to send:', bgError.message);
          }
        });
      }
    } catch (emailError: any) {
      console.error('[signup] ⚠️ Failed to save OTP or enqueue email task:', emailError.message)
    }
