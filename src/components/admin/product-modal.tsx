...
export default function ProductModal({ isOpen, onClose, editingProductId }: ProductModalProps) {
  ...
  const handleAutoSearchAndFill = async () => {
    ...
    setAutoSearchStatus('Searching the internet...')

    try {
      setAutoSearchStatus('Fetching product specifications...')
      ...
      setFormData(prev => ({
        ...prev,
        name: data.name || "", // Overwrite with new name, don't keep old name
        brand: data.brand || "", // Overwrite with new brand or empty, don't keep old brand
        category: data.category || "mobile-phones", // Default to mobile-phones if missing, don't keep old category
        description: data.description || "", // Overwrite description
        // Fill images - overwrite with new images or empty array
        images: data.images || [],
        image: data.images && data.images.length > 0 ? '' : prev.image,
        // Fill specs
        specs: data.specs && Object.keys(data.specs).some((k: string) => data.specs[k])
          ...
          : {}, // Overwrite with empty if no specs found
        // Fill variants
        variants: {
          base_price: Number.parseFloat(prev.price) || 0,
          storage: data.variants?.storage || [],
          ram: data.variants?.ram || [],
